...
class SSQAIndexer:
    def __init__(self, lang):
        ...
        if lang == 'zh':
            logger.info("index directory:{}".format(config.IDX_SSQA_TEXTBOOK))
            indexDir = SimpleFSDirectory(Paths.get(str(config.IDX_SSQA_TEXTBOOK)))
...
def index_all(doc_path):
    ...
    new_dir(config.IDX_SSQA_TEXTBOOK)
    ...
    try:
        ...
        for fp in tqdm(raw_fps):
            ...
            while i < len(input_file):
                ...
                i += 1
                sent_num += 1
        logger.info("Indexed {} lessons.".format(sent_num))
        myIndexer_zh.close()
...
def test_search():
    ...
    with open('gold_test_reseq.tsv', 'r') as inputf, open('searched_test.tsv', 'w') as output:
        questions = csv.DictReader(inputf, delimiter = '\t')
        writer = csv.DictWriter(output, delimiter = '\t', fieldnames = tsvfieldnames)
        output.write("sidx	fid	qid	passages	question	option1	option2	option3	option4	label")
        for q in questions:
            # query to be search
            ...
            #q['passages'] = ret_sents.strip()
            writer.writerow(q)
    
    with open('gold_train_reseq.tsv', 'r') as inputf, open('searched_train.tsv', 'w') as output:
        questions = csv.DictReader(inputf, delimiter = '\t')
        writer = csv.DictWriter(output, delimiter = '\t', fieldnames = tsvfieldnames)
        output.write("sidx	fid	qid	passages	question	option1	option2	option3	option4	label")
        for q in questions:
            ...
            #q['passages'] = ret_sents.strip()
            writer.writerow(q)
    with open('gold_dev_reseq.tsv', 'r') as inputf, open('searched_dev.tsv', 'w') as output:
        questions = csv.DictReader(inputf, delimiter = '\t')
        writer = csv.DictWriter(output, delimiter = '\t', fieldnames = tsvfieldnames)
        output.write("sidx	fid	qid	passages	question	option1	option2	option3	option4	label")
        for q in questions:
